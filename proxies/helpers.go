...
//AddFactoryDefaults lets you add some factory defaults to DirectorFactory for operation
func AddFactoryDefaults(d *DirectorFactory) {
	d.Register("tcp", func(req *ProxyRequest, do *Director, a Action) error {
		var cert *tls.Config
		var err error
		...
		return do.ServeTCP(a, req.From, fmt.Sprintf("%s:%d", req.Addr, req.Port), cert)
	})
	d.Register("httpcon", func(req *ProxyRequest, do *Director, a Action) error {
		return do.ServeHTTPConn(a, req.From, fmt.Sprintf("%s:%d", req.Addr, req.Port), false)
	})
	d.Register("httpreq", func(req *ProxyRequest, do *Director, a Action) error {
		return do.ServeHTTPConn(a, req.From, fmt.Sprintf("%s:%d", req.Addr, req.Port), true)
	})
}
